...
Runnable scripts live in this directory. After a development install
...
The complete example is in [`simple_shadow.py`](simple_shadow.py):
...
[`simple_algo_benchmark.py`](simple_algo_benchmark.py) uses the same scale;
there is no reduced smoke grid yet.
